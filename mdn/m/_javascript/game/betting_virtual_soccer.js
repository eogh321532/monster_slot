...
function toggle($tr)
{
	...
	$(".game").removeClass("m_bet_cellon");
}

// 승무패-무, 언더오버-언더 조합
function check_rule(game_sn, game_template_sn, position)
{
	for(var i=0; i<m_list.length; ++i) 
	{
		var item = m_list[i];
		if(item['game_sn'] == game_sn) {
			if( (game_template_sn == E_1X2 && position == "x") && (item['game_template_sn'] == E_UO && item['position'] == "1") ) {
				return 0;
			}
			else if( (game_template_sn == E_UO && position == "1") && (item['game_template_sn'] == E_1X2 && item["position"] == "x")) {
				return 0;
			}
		}
	}
...
function on_selected_team(node, position)
{
	...
	var game_template_sn = $tr.find('input[name=game_template_sn]').val();
	var count_of_max_betting = $tr.find('input[name=count_of_max_betting]').val();
	var league_sub = $tr.find('input[name=league_sub]').val();
	
	// 승무패는 단폴배팅 가능
	// 언오버는 2폴더 이상부터 배팅가능.
	
	// 특이조건- 승무패의 '무'와 멀티의 언더오버 '언더'는 동시에 배팅불가
	if(!check_rule(game_sn, game_template_sn, position))
	{
		alert("동일경기 무 + 언더 조합은 배팅 불가능합니다.");
		return;
	}
	...
	if(is(game_event_sn, position)) {
		...
		on_delete(game_event_sn);
		if(on_add(game_sn, game_event_sn, home, away, odds_1, odds_x, odds_2, odds_selected, game_template_sn, position, started, count_of_max_betting, league_sub, node)) {
			if(position == '1') {
...
function on_selected_bonus(team, odds_bonus, odds_bonus_min, node)
{
	...
	if(m_odds_bonus == odds_bonus) {
		...
		$('td[name=p_bonus]').each(function(index) {
			var odds = $(this).find(".rate").text();
			if(odds == odds_bonus) {
				$(this).prop('class', 'homepick');
			} else {
...
function display_betting_slip()
{
	...
	if(m_odds_bonus > E_DEFAULT_ODDS_BONUS) {
		...
		$(m_team_bonus_node).addClass("m_bet_cellon");
	...
	for(var i=0; i < m_list.length; ++i) {
		...
		$(item['node']).addClass("m_bet_cellon");
...
function on_add(game_sn, game_event_sn, home, away, odds_1, odds_x, odds_2, odds_selected, game_template_sn, position, started, count_of_max_betting, league_sub, node)
{
