...
function on_selected_team(node, position)
{
	...
	// 
	var is_본경기 = home.indexOf("[")<0;
	for(var i=0; i<m_list.length; ++i) {
		var item = m_list[i];
		if(item['game_league_uid'] != game_league_uid)
			continue;
		if(item['started'] != started)
			continue;
		if(item['home_uid'] != home_uid)
			continue;
		if(item['away_uid'] != away_uid)
			continue;
		if(!is_본경기 && item['home'].indexOf("[")<0)
			is_본경기 = true;
	}
	// 모든경기 본경기+핸디캡은 배팅불가 (동일경기 핸디캡 승무패 조합배팅불가능) 배팅하기누르기전 클릭만해도뜨게끔부탁드려요
	if(is_본경기) {
		for(var i=0; i<m_list.length; ++i) {
			var item = m_list[i];
			if(item['game_league_uid'] != game_league_uid)
			...
			if(item['game_template_sn']==E_1X2 && game_template_sn==E_HDP) {
				if(home.indexOf("[핸디캡") > 0) {
					alert("모든경기 본경기+핸디캡은 배팅불가. (동일경기 핸디캡 승무패 조합배팅불가능)");
					return;
				}
			}
			if(item['game_template_sn']==E_HDP && game_template_sn==E_1X2) {
				if(item['home'].indexOf("[핸디캡") > 0) {
					alert("모든경기 본경기+핸디캡은 배팅불가. (동일경기 핸디캡 승무패 조합배팅불가능)");
					return;
				}
			}
		}
		// 축구 동일경기 언오버 배팅불가 (동일경기 언더/오버 승무패 조합배팅불가능) 배팅하기누르기전 클릭만해도뜨게끔부탁드려요
		if(sport_sn == 4) {
			for(var i=0; i<m_list.length; ++i) {
				var item = m_list[i];
				if(item['game_league_uid'] != game_league_uid)
				...
				if(item['game_template_sn']==E_1X2 && game_template_sn==E_UO) {
					if(home.indexOf("[언더") > 0) {
						alert("축구 동일경기 언오버 배팅불가 (동일경기 언더/오버 승무패 조합배팅불가능)");
						return;
					}
				}
				if(item['game_template_sn']==E_UO && game_template_sn==E_1X2) {
					if(item['home'].indexOf("[언더") > 0) {
						alert("축구 동일경기 언오버 배팅불가 (동일경기 언더/오버 승무패 조합배팅불가능)");
						return;
					}
				}
	...
	// 한경기당 언오버는 1개만 배팅되게 해주세요
	// 한경기에 언오버/핸디캡 한개이상 클릭시 (동일경기 핸디캡 /언오버 중복배팅 불가능합니다) 팝업뛰워주세요 
	if(game_template_sn == E_UO || game_template_sn == E_HDP) {
		for(var i=0; i<m_list.length; ++i) {
			var item = m_list[i];
			if(item['game_league_uid'] != game_league_uid)
				continue;
			...
			if(item['game_sn'] == game_sn)
				continue;
//			if(game_template_sn == E_UO && item['game_template_sn'] == E_HDP) {
//				alert("동일경기 핸디캡 /언오버 중복배팅 불가능합니다");
//				return;
//			}
//			if(game_template_sn == E_HDP && item['game_template_sn'] == E_UO) {
//				alert("동일경기 핸디캡 /언오버 중복배팅 불가능합니다");
//				return;
//			}
			// 같은 언더 및 핸디 선택된거 지우기 위해서.
			if(game_template_sn == E_UO && item['game_template_sn'] == E_UO) {
				on_delete(item['game_event_sn']);
			}
			if(game_template_sn == E_HDP && item['game_template_sn'] == E_HDP) {
				on_delete(item['game_event_sn']);
			}
		}
	}
...
//	}
	...
	if(is(game_event_sn, position)) {
		...
		on_delete(game_event_sn);
		if(on_add(game_sn, game_event_sn, home, away, odds_1, odds_x, odds_2, odds_selected, game_template_sn, position, started, count_of_max_betting, node, game_league_uid, home_uid, away_uid)) {
			if(position == '1') {
...
function display_betting_slip()
{
	...
	if(m_odds_bonus > E_DEFAULT_ODDS_BONUS) {
		...
		var html = 	""+
					...
					"			<div class='cart_bet_x' onclick='on_clear_bonus()'><i class='fa-regular fa-circle-xmark'></i></div>"+
					"		</div>"+
					"		<div class='cart_bet_info'>"+
					"			<div class='bet_bet_team'>"+
					"				<div class='bet_team_name'>"+display_position+"</div>"+
					"				<div class='bet_bet_odd'>"+parseFloat(m_odds_bonus).toFixed(2)+"</div>"+
					"			</div>"+
					"		</div>"+
					"	</div>"+
					"";
...
//					"			<div class='bet_bet_team'>"+
//					"				<div class='bet_team_name'>"+display_position+"</div>"+
//					"				<div class='bet_bet_odd'>"+parseFloat(item['odds_selected']).toFixed(2)+"</div>"+
//					"			</div>"+
//					"		</div>"+
//					"	</div>"+
//					"";
					"<div class='cart_bet'>"+
					"	<div class='cart_bet_title'>"+item['home']+" vs "+item['away']+"<div class='cart_bet_close'><a href='javascript:on_cancel("+item['game_event_sn']+");'><i class='fa-solid fa-xmark'></i></a></div></div>"+
					"	<div class='cart_bet_info'>"+
...
//	$("#open_cart .odds").text(p_odds_selected);
	$(".cart_folder_title SPAN").text(m_list.length);
...
function on_add(game_sn, game_event_sn, home, away, odds_1, odds_x, odds_2, odds_selected, game_template_sn, position, started, count_of_max_betting, node, game_league_uid, home_uid, away_uid)
{
	...
	item['game_league_uid'] = game_league_uid;
	item['home'] = home;
	item['away'] = away;
	item['home_uid'] = home_uid;
	item['away_uid'] = away_uid;
	item['odds_1'] = odds_1;
	item['odds_x'] = odds_x;
	item['odds_2'] = odds_2;
...
function toMore(league_sub_uid) {
	if($(".gamesub_"+league_sub_uid).is(':visible'))
		$(".gamesub_"+league_sub_uid).hide("slow");
	else
		$(".gamesub_"+league_sub_uid).show("slow");
}